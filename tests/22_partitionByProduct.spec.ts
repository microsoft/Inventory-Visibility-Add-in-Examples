...
test("Set partition rule by product ID and add back on-hand", async ({ request }) => {
    test.setTimeout(120000);
    var newConfig = SRConfig;
    newConfig.partitionConfiguration.useDimensions = false;
    newConfig.partitionConfiguration.useProductId = true;
    newConfig.cachePartitionIdVersion = "ByProductId";
...
test("Index query without product ID -- Error", async ({ request }) => {
    var indexQueryBody = {
        "filters": {
            "SiteId": [
                "1"
...
test("Exact query without site-warehouse -- OK", async ({ request }) => {
    var exactQuery = {
        "filters": {
            "dimensions": [
            ],
...
test("Exact query without product ID -- Error", async ({ request }) => {
    var exactQuery = {
        "filters": {
            "dimensions": [
            ],
...
test("Reserve 1 with site and warehouse when partition by product ID -- OK", async ({ request }) => {
...
test("Reserve 1 without site and warehouse when partition by product ID -- OK", async ({ request }) => {
    const reserve = await request.post(
        `${vars.IVQueryPrefix}/onhand/reserve`, {
...
test("Allocate 1 with site and warehouse when partition by product ID -- OK", async ({ request }) => {
...
test("Allocate 1 without site and warehouse when partition by product ID -- OK", async ({ request }) => {
    var d = (new Date()).getTime().toString();
...
test("Schedule 1 piece 3 days later when partition by product ID -- OK", async ({ request }) => {
...
test("Schedule 1 piece 3 days later without site warehouse when partition by Product ID -- OK", async ({ request }) => {
